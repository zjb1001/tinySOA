...
from tinysoa.core.model import Service, Method, Event, Endpoint, Protocol, Message
...
def test_deregister_non_existent_is_noop(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    reg.deregister(999)  # must not raise


def test_find_by_id_returns_none_for_unknown(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    assert reg.find_by_id(999) is None


def test_find_by_name_no_match(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    assert reg.find_by_name("nonexistent") == []


@pytest.mark.asyncio
async def test_invoke_with_service_object_selector(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    inv = InMemoryInvoker(reg)
    resp = await inv.invoke(sample_service, "say", {"k": "v"})
    assert resp.payload["method"] == "say"


@pytest.mark.asyncio
async def test_invoke_with_method_object_selector(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    inv = InMemoryInvoker(reg)
    method = sample_service.methods[0]
    resp = await inv.invoke(1, method, {"k": "v"})
    assert resp.payload["method"] == "say"


@pytest.mark.asyncio
async def test_publish_with_message_payload(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    bus = InMemoryBus(reg)
    received = []
    ...
    sub = bus.subscribe("echo", "said", handler)
    msg = Message(payload={"wrapped": True}, headers={"h": "1"})
    await bus.publish("echo", "said", msg)
    assert received[0] == {"wrapped": True}
    bus.unsubscribe(sub)


@pytest.mark.asyncio
async def test_publish_with_service_object_selector(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    bus = InMemoryBus(reg)
    received = []
    ...
    bus.subscribe(sample_service, "said", handler)
    await bus.publish(sample_service, "said", {"v": 1})
    assert len(received) == 1


def test_subscribe_unsubscribe_non_existent_is_safe():
    sub = Subscription.new(service_id=999, event_id=999)
    reg = InMemoryRegistry()
    bus = InMemoryBus(reg)
...
@pytest.mark.asyncio
async def test_invoke_method_id_not_found_raises(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    inv = InMemoryInvoker(reg)
    with pytest.raises(NotFoundError):
        await inv.invoke(1, 999, {"k": "v"})


@pytest.mark.asyncio
async def test_subscribe_with_event_object_selector(sample_service):
    reg = InMemoryRegistry()
    reg.register(sample_service)
    bus = InMemoryBus(reg)
    received = []
    ...
    event_obj = sample_service.events[0]
    bus.subscribe(sample_service, event_obj, handler)
    await bus.publish(sample_service, event_obj, {"e": "ok"})
    assert len(received) == 1
