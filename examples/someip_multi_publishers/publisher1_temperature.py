#!/usr/bin/env python3
"""SOME/IP temperature sensor publisher (engineering-grade).

Periodically publishes simulated temperature readings on the
``sensor.temperature`` topic via a SOME/IP EventGroup, advertising the service
...
directory):
    ...
    python publisher1_temperature.py                 # run forever
    python publisher1_temperature.py --count 5       # bounded (CI-friendly)
...
def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="tinySOA SOME/IP temperature publisher")
    parser.add_argument(
