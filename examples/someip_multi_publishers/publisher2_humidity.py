#!/usr/bin/env python3
"""SOME/IP humidity sensor publisher (engineering-grade).

Periodically publishes simulated humidity readings (clamped to 0-100%) on the
``sensor.humidity`` topic via a SOME/IP EventGroup, advertising the service
...
directory):
    ...
    python publisher2_humidity.py                 # run forever
    python publisher2_humidity.py --count 5       # bounded (CI-friendly)
...
def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="tinySOA SOME/IP humidity publisher")
    parser.add_argument(
